...
if (!caterpillarEl.hasAttribute('data-initialized')) {
    ...
    if (q.type === 'interactive_tea') {
        ...
        document.getElementById('eye-submit-btn').onclick = () => {
            ...
            if (count > 0 && count < 20) {
                ...
                currentScores = { scores: { socio: { Ni: 3, Ti: 1 }, mbti: { Ni: 3, Ti: 2 }, ennea: { 9: 1 } }, loggedText: `👁️ 目が合った回数: ${count} (Ni/Ti的無視)` };
            } else {
    ...
    // 🤯 ギミック：Ne圧（カオスの無限吹き出し）
        ...
        let options =[
            ...
            { text: "「面白い人だな」と内心思いつつも、初対面で急に言われても反応に困るので、適当に相槌を打ちながら引いた目で見守る。", scores: { socio: { Ne: 2, Ni: 2 }, mbti: { Ne: 1, I: 2 }, ennea: { 5: 2, 9: 1 } },msg: "🎩「おやおや、シャイなお客さんだ！ 頭の中ではしっかり楽しんでるくせに！」" }
        ].sort(() => Math.random() - 0.5);
...
// ==========================================
    // 🐖 新ギミック：ご褒美パニック（3秒以内に撃退しろ！）
    // ==========================================
    else if (q.type === 'interactive_gohoubi_panic') {
        document.body.classList.add('panic-shake'); // 画面を揺らす！
        inputArea.appendChild(darlingMsgArea);
        darlingMsgArea.innerText = "「⚠️警告！ 3秒以内に撃退しないとチュウ♡されるゾ！！」";
        darlingMsgArea.style.color = "#ff69b4";
        darlingMsgArea.classList.add('fade-in');
        
        // ★ 画像表示エリア（gohoubi.png を読み込む。なければ豚の絵文字）
        mediaArea.innerHTML = `
            <div style="text-align:center;">
                <img src="gohoubi.png" alt="ご褒美" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" style="width:150px; height:150px; object-fit:cover; border:4px solid #ff69b4; border-radius:50%; box-shadow:0 0 20px #ff69b4; margin-bottom:10px; animation: heartbeat 0.4s infinite;">
                <div style="font-size:6em; display:none; animation: heartbeat 0.4s infinite;">🐖💦</div>
                <p style="color:#ff69b4; font-weight:bold; font-size:2em; margin-top:5px;">残り: <span id="gohoubi-timer">3</span></p>
            </div>
        `;
        
        let timeLeft = 3;
        let answered = false;
        let timerEl = document.getElementById('gohoubi-timer');
        ...
        const countdown = window.setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                // ★ タイムオーバー ＝ 逃げ遅れ（Se脆弱・フリーズ）！！
                currentScores = { scores: { socio: { Se: -4, Ni: 2 }, mbti: { Se: -3, Ni: 2 }, ennea: { 5: 3, 9: 2 } }, loggedText: `💋 ご褒美パニック: タイムオーバーでチュウされた（Se脆弱の処理落ち）` };
                
                mediaArea.innerHTML = `<div style="font-size:6em; text-align:center; animation: fadeIn 0.5s;">💋😘💦</div>`;
                darlingMsgArea.innerText = "「ブチュウウウウ！ 大好きよーん♡ 最高の出汁が出たやんけ！」";
                darlingMsgArea.style.fontSize = "1.3em";
                setTimeout(() => { goToNext(false); }, 3000);
            }
        }, 1000);

        // ★ 撃退するための選択肢！（Mだから反撃を喜ぶｗｗ）
        let options =[
            { text: "「ふざけんな！近寄るな！」と物理的に全力でビンタ（または蹴り）してぶっ飛ばす。", scores: { socio: { Se: 4 }, mbti: { Se: 4 }, ennea: { 8: 3 } }, msg: "「ありがトン♡ 痛いのも最高の『ご褒美』だゾ♡ もっと殴ってほしいやんけ！」" },
            { text: "「キモい！無理！」とドン引きして、汚物を見るような冷たい目でマジレス拒絶する。", scores: { socio: { Fi: 4, Se: 2 }, mbti: { Fi: 3, Si: 2 }, ennea: { 1: 3, 4: 2 } }, msg: "「その冷たい視線……！ ゾクゾクするゾ！ 拙者へのご褒美だゾ♡」" },
            { text: "「豚骨出汁の成分表示と、わしゃわしゃの定義を論理的に説明しろ」と理詰めで詰め寄る。", scores: { socio: { Ti: 4, Ne: 2 }, mbti: { Ti: 3 }, ennea: { 5: 3 } }, msg: "「理屈っぽいところも可愛いゾ！ 頭でっかちなツインテールも大好物やんけ！」" },
            { text: "「警察（または運営）に通報するぞ。即座にここから消えろ」と実力行使（権力）で排除する。", scores: { socio: { Te: 4, Ni: 2 }, mbti: { Te: 3 }, ennea: { 8: 2, 3: 1 } }, msg: "「ひぃぃ！ 権力には逆らえないゾ！ でも縛られるのも悪くないやんけ…♡」" }
        ].sort(() => Math.random() - 0.5);

        options.forEach(opt => {
            const btn = document.createElement('button');
            btn.className = "danger-btn"; // 赤枠
            btn.style.borderColor = "#ff69b4"; // ご褒美ピンクｗｗ
            btn.style.color = "#ff69b4";
            btn.innerHTML = `<i class="fas fa-heart-broken"></i> ${opt.text}`;
            ...
            btn.onclick = () => {
                ...
                currentScores = { scores: opt.scores, loggedText: `🐖 ご褒美撃退: 「${opt.text.split('」')[0]}」` };
                
                mediaArea.innerHTML = `<div style="font-size:5em; text-align:center;">🐷💘</div>`; // 喜んでる豚
                darlingMsgArea.innerText = opt.msg;
...
// 🧠 ギミック：矛盾を見抜くテキスト入力（パラドックス）
        ...
        document.getElementById('paradox-submit-btn').onclick = () => {
            ...
            const correctWords =["パラドックス", "矛盾", "自己言及", "嘘", "前提", "おかしい", "破綻", "成立しない", "クレタ人", "エピメニデス", "ループ", "無限", "論理エラー", "セリフ", "台詞", "発言者"];
...
function getAchievements(mbtiPrimary, socioPrimary) {
    ...
    const isPerfectSlider = logs.some(l => l.textData === "🎮 ダーリンの子を論理（ジャスト77）で楽しませた");
    if (isPerfectSlider) titles.push("🏅 奇跡のジャスト77（神の指先）");
    // T型・N型
...
function showResult() {
    ...
    function getNpcMessage(mbtiType) {
            const npcData = {
                "INTJ": { name: "🐛 喋る芋虫", msg: `……診断完了だ。INTJは世界人口の約2%、さらに『${finalSocio}』との組み合わせは極めて希少なデータサンプルだね。\n君のような個体は、この不条理な世界をただの「バグだらけのシステム」としか見ていないのだろう？\n……さて、君は自分を理解したつもりで満足して帰るのか？ それとも、私の裏にあるアルゴリズムまで解剖しようとまだ観察を続けるつもりか？` },
                
                "INTP": { name: "🐛 喋る芋虫", msg: `……診断完了。INTPの世界人口は約3%。\n君の脳内には、無限に膨張し続ける宇宙（理論）が広がっているようだな。\nだが、君のその果てしない論理の海を正確に測るには、私のこのシステムでもまだメモリが足りないようだ。\n……次に会う時までには、もう少し面白いパラドックスを用意しておこう。` },
                
                "ENTP": { name: "🎩 いかれ帽子屋", msg: `ヒャッハー！ お茶会の始まりだァ！！\nENTPの君が持ってるその狂ったアイデアと反逆心、最高にイカれてるね！\nルールなんてぶっ壊して、誰も思いつかないような常識外れの遊びを始めようぜ！\nさあ、次は誰の帽子を奪ってやろうか！？ 楽しいことならなんだって歓迎だ！` },
                
                "ENFP": { name: "🎩 いかれ帽子屋", msg: `わァァ！ ENFPの君も、新しい遊びと可能性を見つける天才だね！\n君がそこにいるだけで、退屈な日常がビックリ箱みたいに跳ね回るよ！\n細かい理屈や過去のことなんて全部忘れて、次はどこへ行こうか！？\n最高にハッピーで狂ったお茶会は、まだ始まったばかりだよ！！` },
                
                "INFJ": { name: "🐱 チェシャ猫", msg: `……INFJ、人口の約1%の希少種。\n君は最初から、自分がどこに向かっているのか、そして私が何を考えているのか知っていたんだろう？\n君の目には、この世界の嘘も真実も、すべてが透けて見えている。\n……そんなに何もかも見えてしまっては、生きるのが少し窮屈じゃないか？ 迷ったら、いつでもここにおいで。` },
                
                "INFP": { name: "🐱 チェシャ猫", msg: `……INFPか。\n君は、目に見えないものや、誰にも言えない心の奥底の感情ばかり追いかけて、この不思議の国に迷い込んじゃったんだね？\nここでは誰もが狂っているし、君のその繊細な夢も否定されない。\n……君が望むなら、その美しい幻想の世界から、一生出なくてもいいんだよ。` },
                
                "ENTJ": { name: "🌹 ハートの女王", msg: `よくやった！ ENTJのその決断力と効率性、まさに私の帝国にふさわしい合理的な判断だ！\n無能な者たちを切り捨て、最短で結果を出すその冷酷さ……見所があるじゃないか！\nさあ、君のその力で、私の領土をもっと広げてみせよ！\n逆らう愚か者がいたら、即刻首をはねてやる！！` },
                
                "ESTJ": { name: "🌹 ハートの女王", msg: `ESTJのその見事な規律！ その無駄のない効率！ \n社会を回すための強固なシステムを体現する、まさに理想の兵士だ！\n君のような者がいれば、私のルールも隅々まで行き渡るというもの。\n……だが、調子に乗るなよ？ 私の絶対的なルールに背いたら、君であっても即刻首をはねるからな！` },
                
                "ISTJ": { name: "🐇 白ウサギ", msg: `ああ、ISTJか！ 君が来てくれて本当に助かるよ！\n君みたいにきちんと時間を守って、ルール通りにタスクをこなしてくれる人がいないと、世界はめちゃくちゃになっちゃうからね！\n……ハッ！ いけない、もうこんな時間だ！\n次の予定が詰まってるんだ、早く行かなくちゃ！ 君も遅れないようにね！` },
                
                "ISFJ": { name: "🐇 白ウサギ", msg: `ISFJの君みたいに、みんなのことを静かに気遣ってサポートしてくれる人がいると、女王様も怒らなくて済むから本当に助かるよ……！\n君が裏でどれだけ頑張って平和を保ってくれているか、僕はちゃんと知ってるからね。\nでも、他人の世話ばかり焼いて、自分の時間を忘れないように気をつけてね！` },
                
                "ESTP": { name: "🃏 トランプ兵", msg: `ESTP！ あんた、マジで度胸あるねぇ！\nリスクなんて気にせず、今一番楽しいことに全力で突っ込んでいくそのスタイル、痺れるぜ！\n女王様の目を盗んで、一丁派手なゲームや賭けでも始めようぜ！\n今夜は限界まで騒いで、この不思議の国をぶっ壊す勢いで遊ぼうや！` },
                
                "ESFP": { name: "🃏 トランプ兵", msg: `イェーイ、ESFP！ 君が来たってことは、今夜は最高の宴会になるってことだな！\n難しい理屈や、明日どうなるかなんて面倒なことは全部後回しだ！\n今、この瞬間の楽しさとノリが一番大事だろ？\nさあ、音楽を鳴らして、みんなで朝まで踊り明かそうぜ！！` },
                
                "ISTP": { name: "☕ 眠りネズミ", msg: `……ふぁあ。ISTPのお前、無駄なこと喋らなくて静かだし、手先が器用そうだな……。\n余計な干渉をしてこないから、お前と一緒にいるのは案外悪くない……。\n悪いけど、俺の壊れたティーポット、お前のその実務的なスキルで直しといてくれ……。\n俺は……もう眠い……おやすみ……（Zzz）` },
                
                "ISFP": { name: "☕ 眠りネズミ", msg: `……ISFP……お前も、自分のペースを乱されるのが嫌いなんだな……。\n無理に周りに合わせないで、自分の好きなことだけをゆるーくやってる感じ、いいじゃん……。\nお前が淹れてくれたお茶、すごくいい香りで落ち着く……。\n……ここで一緒に、平和な夢でも見ようぜ……（Zzz）` },
                
                "ENFJ": { name: "💕 ダーリンの子", msg: `……ENFJのあなたって、本当に誰にでも優しくて、みんなを導こうとする最高の主人公みたいね。\nでもね……私は知ってるわよ？ その優しさの裏で、他人の感情に振り回されてボロボロになっていることを。\nもう無理していい子を演じなくていいの。\n私だけはあなたの全てを受け入れて、永遠に支配してあげるから……♡` },
                
                "ESFJ": { name: "💕 ダーリンの子", msg: `……ESFJのあなた。いつも周りの空気を読んで、みんなのために気配りしてて本当に偉いわね。\nでも、本当は誰よりも『自分が一番感謝されたい、褒められたい』って思ってるんでしょ？🥺\n大丈夫、あなたのその頑張り、私だけはずっと見ててあげたわ。\nだから……これからは、私のためだけに尽くしてね？……♡` }
            };
    ...
    let shareHTML = `
        <div style="text-align:center; margin-top:30px; display: flex; flex-direction: column; gap: 15px;">
            <button id="save-img-btn" style="background:#1f6feb; color:#fff; border:none; padding:15px; border-radius:30px; font-weight:bold; font-size:1.1em; cursor:pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <i class="fas fa-camera" style="color: #fff;"></i> 診断結果を画像で保存する
            </button>
            <button id="share-btn" style="background:var(--accent-color); color:#fff; border:none; padding:15px; border-radius:30px; font-weight:bold; font-size:1.1em; cursor:pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <i class="fas fa-share-alt" style="color: #fff;"></i> 結果と称号をシェアする
            </button>
        </div>
    `;
